...
const Content = memo(({ showTodo, isList, isSlidebarOpen }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full">
      {/* Slidebar section */}
      <div
        className={`transition-transform transform ${isSlidebarOpen ? 'translate-x-0' : '-translate-x-full'} 
        w-full lg:w-[280px] lg:block bg-gray-100 dark:bg-gray-900 shadow-md duration-300 lg:translate-x-0`}
      >
        <Slidebar uncompletedTasks={uncompletedTasks} completedTodos={completedTodos} />
      </div>

      {/* Main content section */}
      <div className={`w-full lg:flex-grow relative p-4 lg:p-8 transition-all duration-300`}>
        <button
        ...
        {isOpen && showTodo && (
          <div className="w-full bg-gradient-to-b from-[#fbfdfc] to-[#D4EFD7] dark:bg-gradient-to-b dark:from-[#2f3630] dark:to-[#1f261b] my-1 p-4 space-y-4 md:space-y-6">
            ...
            <div className="flex text-3xl font-extrabold justify-between">
              ...
              <div>
                <button onClick={toggleDiv} className="text-xl font-semibold bg-green-500 rounded p-2 w-40 h-14">
                  Add Task
                </button>
              </div>
            </div>
              ...
              <div className="h-fit dark:text-white">
                <div className="space-y-1 text-black dark:text-white">
                  <TodoProvider value={{ todos, addTodo, deleteTodo, toggleComplete }}>
                    ...
                    <div
                      className={`${isList ? 'block' : 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4'}`}
                    >
      ...
      {/* Additional Divbar section */}
      <div className="hidden lg:block m-4 md:m-8 lg:m-16 w-full lg:w-[280px]">
        <Divbar />
      </div>
    </div>
  );
