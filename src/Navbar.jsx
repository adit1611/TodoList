...
const Navbar = memo(({ toggleLayout, isList, toggleSidebar }) => { // Ensure toggleSidebar is passed as a prop
    ...
    return (
        <div className="navbar flex justify-between items-center w-screen h-[10vh]">
            <div className="left flex items-center w-1/2 space-x-1">
                <IoMenu 
                    className="text-6xl mx-20 cursor-pointer" 
                    onClick={toggleSidebar} // Add onClick event to toggle sidebar
                />
            ...
            <div className="right flex items-center justify-end w-1/2 space-x-5 mr-10">
                ...
                <button onClick={toggleLayout} className="relative top-3 p-2 rounded">
                    {isList ? <HiOutlineViewGrid className='text-4xl'/> : <HiListBullet className='text-4xl'/>}
                </button>
                <button onClick={toggleTheme} className="relative top-3 p-2">
